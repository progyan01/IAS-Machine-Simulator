...
#Calculate line length: 40 bits + newline characters (1 for Unix, 2 for Windows)
LINE_LENGTH = 40 + len(os.linesep)
...
#Helper functions for Disk-as-RAM access
def read_memory(address):
    """Reads a specific line from binary.txt"""
    if not os.path.exists(BINARY_FILE):
        return '0' * 40
        
    with open(BINARY_FILE, 'r') as f:
        f.seek(address * LINE_LENGTH)
        line = f.read(40)
        #Return 40 zeros if address is uninitialized/empty
        if len(line) < 40: return '0' * 40 
        return line

def write_memory(address, binary_string):
    """Writes a specific line to binary.txt"""
    with open(BINARY_FILE, 'r+') as f:
        f.seek(address * LINE_LENGTH)
...
def execute():
    global AC, MQ, MBR, MAR, IR, PC, IBR

    #Fetch Data for Arithmetic Ops
    if IR in [1, 5, 6, 11, 12]: 
        MBR = to_signed(int(read_memory(MAR), 2))
    ...
    if IR == 0:            #NOP
        ...
        write_memory(MAR, format(to_unsigned(AC), '040b'))
        
    elif IR == 18:         #STOR_L M(X, 8:19)
        current_word = int(read_memory(MAR), 2)      #This is the "Self-Modifying" instruction
        ...
        #Write directly to file
        write_memory(MAR, format(new_word, '040b'))
...
if __name__ == "__main__":
    ...
    #Dump Final Memory for Verification (Reading from file)
